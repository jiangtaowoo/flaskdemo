...
def trans_log(logmsg):
    ...
    logfilename = os.sep.join([cur_dir,'logs','trans_{0:%Y-%m-%d}.log'.format(datetime.datetime.today())])
...
class WordTranslateResult(object):
    ...
    def export_db(self, filters, cboper=None):
        ...
        if cboper and isinstance(cboper,list) and dataset:
            ...
            with open(filename, 'a+') as outf:
                for datarow in dataset:
                    set_datarow(fieldnames, datarow)
                    outf.write(self.make_flashcard().encode('utf-8'))
                    outf.write('\n')
            """
            return True
        elif dataset:
    ...
    #生成HTML卡片结果数据
    def gen_htmlcard_data(self, filters):
    ...
    def gen_flashcard_data(self):
        ...
        if self.means:
            ...
            dictname_raw = {'oxford': u'{0}牛津词典{1}'.format("<b>","</b><br>"),
                        'collins': u'{0}柯林斯词典{1}'.format("<b>","</b><br>")}
            ...
            for dname, dmeans in self.means.iteritems():
                if dname in dictname:
                    ...
                    res_dict[dname] = u"<div>{0}</div>".format(''.join(dict_mean))
        #第七行, 英文释义
        back_idx = len(back)
        if self.enmeans:
...
class BDTranslation(object):
    ...
    def _translate_request(self, word):
        ...
        if not word_in_dict:
            ...
            try:
                ...
                rsp = self.sess.get(**self.req_dict_v)
                res = word_obj.parse_vocabulary_result_from_rsp(rsp)
                if not res:
                    return None
                #fanyi.baidu
    ...
    def translate(self, word):
        ...
        if result:
            BDTranslation._log_translate_result('{0} OK!'.format(word))
            return result
        else:
            BDTranslation._log_translate_result('{0} fail ...'.format(word))
            return {}

    def query(self, pattern):
        pattern = pattern.replace('*','%')
        word_obj = WordTranslateResult()
        res = word_obj.gen_htmlcard_data({"word":pattern})
        return res
